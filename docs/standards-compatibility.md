...
9. **Implemented for the `0.4.0` scope:** official temporary React Native `0.87.1` and `0.86.3` hosts type-check and bundle the fixture through primitive, typed-adapter, and closed-variant catalogs. Historical Android TalkBack and iOS XCUITest results remain scoped to that fixture; current library-specific work proceeds in the non-blocking Expo Go PoCs.
