...
| Component ownership | A2UI catalogs constrain available components and functions                                                           | Pinned basic catalog plus explicit host component/event/function allowlists                                                          | Partial, fail-closed policy  |
...
| Component graph     | A2UI uses catalog-defined components and ID references rooted at component ID `root`                                 | Validation plus native-plan adaptation for the documented static component subset                                                    | Partial adapter              |
| Data model          | Dynamic values use JSON Pointer bindings and renderer-local state                                                    | Ordered updates plus resolved absolute bindings in the static native adapter                                                         | Partial                      |
| Actions             | Renderer-to-agent action envelopes include surface, source component, timestamp, and resolved context                | Static-plan event context is resolved; mounted dispatch and envelope timestamps remain deferred                                      | Partial plan data            |
...
| Capabilities        | Supported catalogs and inline-catalog support are negotiated through transport metadata or initialization            | Pinned basic catalog is policy-gated locally; renderer capability exchange remains deferred                                          | Partial                      |
| Accessibility       | Explicit accessibility data overrides inferred defaults                                                              | Static adapter resolves explicit label, description, live-region, and hidden fields; button and input labels have inferred fallbacks | Partial                      |
...
4. **Partial:** validate the pinned catalog, explicit host component/event/function allowlists, rooted graph references and cycles, binding syntax and template context, selected placement rules, and bounded nested expressions in literal `formatString` sources against reconstructed pinned-catalog calls; runtime-provided format sources, dynamic execution, platform accessibility behavior, and renderer capabilities remain deferred.
5. **Partial:** adapt `Row`, `Column`, static `List`, `Card`, `Text`, `Button`, and `TextField` plus absolute bindings into the renderer's internal trusted plan; dynamic templates, functions, unsupported components, and event mounting fail closed.
