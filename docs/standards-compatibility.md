...
protected HTTP OAuth boundary is verified; the native-library demonstration matrix remains an open
app-level track. See the [MCP protocol support policy](protocol-support.md), [A2UI
...
| MCP authorization   | `2026-07-28` OAuth discovery, PKCE, issuer/resource binding, least-privilege scopes, and bearer usage     | Issuer-bound secure-store/callback provider, persistent resource-bound scope history, and credential-safe transport factory passing all 25 scored pinned authorization client scenarios; app-level native demonstration matrix remains open                                 | Verified package boundary               |
...
9. **Implemented beginning in the `0.7.0` release:** official temporary React Native hosts type-check and bundle the accessibility and complete non-media fixtures through primitive, typed-adapter, and closed-variant catalogs. The current exact boundaries are `0.87.1` and `0.87.0`. The fixture intentionally uses a no-network image placeholder because the raw React Native image primitive cannot enforce the declared redirect/decode/cache grant. The Expo Go integration-app matrix remains open for platform and library-specific validation.
...
11. **Current React Native boundary policy:** generated-host CI bundles and natively builds the exact declared minimum `0.87.0` and the current latest `0.87.1` on pull requests. The latest pin rotates as React Native releases advance; the minimum pin changes only with an explicit peer-range compatibility decision. The previous `0.86.0` minimum was raised after its runtime-factory API proved incompatible with the pinned community JavaScriptCore package. iOS JSC lanes build React Native core and dependencies from source; Hermes lanes use the standard prebuilt path.
12. **Implemented in the `0.9.0` release:** the generated reference host combines native A2UI, a local Fabric extension, and an isolated MCP Apps sibling. Its host-owned coordinator distributes lifecycle and environment state without merging protocol, WebView-security, or accessibility-tree boundaries. The exact React Native lanes run with both Hermes and community JavaScriptCore; see the [support matrix](support-matrix.md).
...
7. **Verified package boundary:** all 25 scored authorization client scenarios in the exact pinned
   ...
   tracked separately through the open app-level demonstration matrix; broader host controls remain host
   responsibilities.
