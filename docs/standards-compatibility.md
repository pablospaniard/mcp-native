...
| Wire envelopes      | `version: "v1.0"` messages use the direction-specific pinned envelope unions                              | Schema-validated lifecycle JSONL plus owned parsing for every renderer-to-agent kind; agent-initiated renderer-function execution is excluded                                                                                                                                                                                                                        | Supported profile            |
...
8. **Implemented for the declared profile:** fixed-seed generated inputs exercise both envelope directions, ordered lifecycle state, bounded dynamic-list plans, and hostile mutations; documented Node.js regression budgets cover maximum-size parsing, rapid updates, render-plan construction, and retained heap. Real-device performance remains part of the platform matrix.
9. **Implemented for the `0.4.0` release scope:** official temporary React Native `0.87.1` and `0.86.3` hosts type-check and bundle the fixture through primitive, typed-adapter, and closed-variant catalogs; Android/iOS native build preflight, a WCAG 2.2 applicability assessment, and a strict evidence validator are defined. The passing accessibility gate covers React Native `0.87.1` on an Android 17 Google Play emulator with TalkBack and an iOS 26.5 simulator through XCUITest accessibility APIs. This does not claim VoiceOver, physical-device, minimum-OS, or React Native `0.86` accessibility coverage.
