...
host-adapter profile. An interactive OAuth foundation is implemented without claiming the complete
protected HTTP profile. See the [MCP protocol support policy](protocol-support.md), [A2UI
...
| MCP authorization   | `2026-07-28` OAuth discovery, PKCE, issuer/resource binding, least-privilege scopes, and bearer usage     | Official SDK flow behind an issuer-bound secure-store/callback provider and credential-safe transport factory; official auth scenarios and platform evidence remain pending                                                                                                                                                                                          | Implemented foundation; not conformant  |
...
| Component graph     | A2UI uses catalog-defined components and ID references rooted at component ID `root`                      | Validation plus native-plan adaptation for the documented subset and bounded dynamic `List` template expansion                                                                                                                                                                                                                                                       | Implemented declared profile            |
| Data model          | Dynamic values use JSON Pointer bindings and renderer-local state                                         | Ordered agent updates plus reactive local string writes for absolute and dynamic-list-relative bindings; `@index` is scoped to template items                                                                                                                                                                                                                        | Implemented declared profile            |
...
| Accessibility       | Explicit accessibility data overrides inferred defaults                                                   | Mounted supported components map label, description, live-region, and hidden fields; the host boundary derives closed text/button roles, button disabled state, hidden focus exclusion, font scaling, and button/input label fallbacks; pinned RN 0.87/0.86 hosts, a WCAG assessment, native build preflight, and a strict two-environment evidence gate are defined | Implemented supported subset            |
...
4. **Implemented for the declared profile:** validate the pinned catalog, explicit host component/event/function allowlists, rooted graph references and cycles, binding syntax and template context, selected placement rules, and bounded nested expressions in literal `formatString` sources against reconstructed pinned-catalog calls; execute bounded formatting, plural, boolean, validation, HTTP(S) `openUrl`, and supported renderer checks. The `0.4.0` platform accessibility matrix passes.
5. **Implemented for the declared profile:** adapt and mount `Row`, `Column`, static and dynamic `List`, `Card`, `Text`, `Button`, and `TextField`; update renderer-local state through absolute and template-relative string bindings; evaluate supported formatting, boolean, validation, and template-scoped `@index` functions with expansion limits and a host-owned locale; expose failed field checks and disable invalid buttons; select supported accessibility semantics; derive closed roles, disabled state, and text scaling; preserve closed host-owned structural and style variants; resolve template-instance events against current state; and execute canonical HTTP(S) `openUrl` through a synchronous host policy. All other server-controlled inputs fail closed. The [native accessibility test plan](native-accessibility-testing.md) records passing Android TalkBack and iOS XCUITest rows.
...
7. **Implemented for the declared profile:** official protocol examples, schema-derived bidirectional fixtures, malformed-message cases, lifecycle tests, exact exclusions, and Candidate interpretations are recorded in the [conformance profile](a2ui-v1-conformance.md).
...
9. **Implemented for the `0.4.0` release scope:** official temporary React Native `0.87.1` and `0.86.3` hosts type-check and bundle the fixture through primitive, typed-adapter, and closed-variant catalogs; Android/iOS native build preflight, a WCAG 2.2 applicability assessment, and a strict evidence validator are defined. The passing accessibility gate covers React Native `0.87.1` on an Android 17 Google Play emulator with TalkBack and an iOS 26.5 simulator through XCUITest accessibility APIs.
...
4. **Implemented transport policy:** protected transports reject manual Authorization, Cookie, and
   Proxy-Authorization headers and surface insufficient-scope responses to the host, preserving a
   consent boundary before step-up authorization.
...
6. **Not yet claimed:** the complete protected Streamable HTTP profile remains pending until every
   applicable pinned official authorization client scenario and real-platform storage/session gate
   passes.
...
- Base compatibility statements on passing pinned conformance and integration evidence.
