...
11. **Current React Native boundary policy:** generated-host CI bundles and natively builds the exact declared minimum `0.86.0` and the current latest `0.87.1` on pull requests. The latest pin rotates as React Native releases advance; the minimum pin changes only with an explicit peer-range compatibility decision.
