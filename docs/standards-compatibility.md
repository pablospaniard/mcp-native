...
| Component ownership | A2UI catalogs constrain available components and functions                                                | Every non-media basic-catalog component, explicit host component/event/function allowlists, installed-subset advertising, and typed local adapters for host or third-party implementations                                                                                          | Implemented closed profile              |
...
| Component graph     | A2UI uses catalog-defined components and ID references rooted at component ID `root`                      | Validation plus native-plan adaptation for every non-media basic-catalog component and bounded dynamic `List` template expansion                                                                                                                                                    | Implemented declared profile            |
...
| Accessibility       | Explicit accessibility data overrides inferred defaults                                                   | Mounted components map explicit metadata and derive closed text, button, image, checkbox, radio/tab, and adjustable semantics; modal focus/back/restoration remains an explicit installed-host responsibility; component-library compatibility is explored in separate Expo Go PoCs | Implemented non-media subset            |
...
5. **Implemented for the declared profile:** adapt and mount every non-media basic-catalog component; update renderer-local state through typed absolute and template-relative bindings; evaluate supported formatting, boolean, validation, and template-scoped `@index` functions with expansion limits and a host-owned locale; require per-image grants inside cumulative image-count, transfer-byte, and decoded-pixel budgets; expose failed checks and disable invalid buttons; derive closed accessibility semantics; preserve closed host-owned variants; resolve template-instance events against current state; and execute canonical HTTP(S) `openUrl` through a synchronous host policy. `Video`, `AudioPlayer`, unknown components, SVG icon payloads, raw styles, and all other unsupported server-controlled inputs fail closed. The [Expo Go PoC policy](native-accessibility-testing.md) defines separate app-level component-library demonstrations.
