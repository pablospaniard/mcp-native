...
This document maps MCP Native to the exact MCP, A2UI, MCP Apps, accessibility, and platform profiles
verified by the finalized v1 implementation. Use it to see the implemented coverage, normative pins,
and the evidence behind each compatibility statement.
...
protected HTTP OAuth boundary is verified, and the [Expo Go todo app](../examples/expo-go-todolist/README.md)
provides a runnable native application example. See the [MCP protocol support policy](protocol-support.md), [A2UI
...
v1.0 as Candidate and v0.9.1 as the current production release. MCP Native
supports only the [pinned, feature-scoped v1.0 Candidate profile](a2ui-v1-conformance.md).
...
form profile at the existing A2UI pin. It runs on the current React Native implementation only;
...
| MCP data fidelity   | Official tools, content, resources, schemas, metadata, annotations, and cache hints                       | Preserved across the documented tools/list, tools/call, and resources/read boundary                                                                                                                                                                                             | Verified documented boundary            |
| MCP authorization   | `2026-07-28` OAuth discovery, PKCE, issuer/resource binding, least-privilege scopes, and bearer usage     | Issuer-bound secure-store/callback provider, persistent resource-bound scope history, and credential-safe transport factory passing all 25 scored pinned authorization client scenarios                                                                                         | Verified package boundary               |
...
| Component ownership | A2UI catalogs constrain available components and functions                                                | All 18 pinned basic-catalog component names within profile limits, explicit host component/event/function allowlists, installed policy-ready subset advertising, typed local adapters, and closed locally compiled extension registrations                                      | Implemented closed profile              |
...
| Validation          | A2UI v1.0 messages and catalogs validate against its JSON Schemas                                         | Pinned Candidate schemas via Ajv for supported lifecycle and every renderer-to-agent message kind                                                                                                                                                                               | Supported profile                       |
...
| Component graph     | A2UI uses catalog-defined components and ID references rooted at component ID `root`                      | Validation plus native-plan adaptation for all 18 pinned basic-catalog components within profile limits, bounded dynamic `List` template expansion, and negotiated leaf-only host extensions                                                                                    | Implemented declared profile            |
...
| Host action consent | Product policy rather than a server-granted protocol capability                                           | Core dispatch and optional direct calls support bounded review and expiring/revocable grants; the high-level host supplies one immutable, serialized, fail-closed decision boundary to the separate MCP Apps and A2UI policy callbacks; server metadata remains non-authorizing | Implemented host boundary               |
...
The package implements the exact [A2UI conformance profile](a2ui-v1-conformance.md) as part of
the finalized v1 API. A2UI is transport-agnostic and defines a stream of protocol envelopes;
MCP Native combines that envelope model with the negotiated project binding below.

MCP Native defines a [project-owned A2UI-over-MCP transport binding](a2ui-mcp-binding.md). It negotiates exact settings under `io.github.pablospaniard/mcp-native-a2ui` and defines ordered JSONL resource transport for the pinned Candidate revision. The package parses lifecycle envelopes into a bounded ordered store, parses every renderer-to-agent message kind as owned non-authorizing data, requires pinned-catalog plus host-policy validation for renderer-ready snapshots, validates closed agent/renderer capability objects, negotiates exact shared catalog IDs, and constructs official renderer actions. The React Native package mounts the supported semantics of all 18 pinned basic-catalog components with bounded dynamic lists, typed renderer-local bindings, required image/media grants, installed-subset advertising, dispatch-time template event context, and the separately negotiated [host-extension profile](media-and-host-extensions.md).

### `@mcp-native/react-native`

The trusted render plan and host-owned renderer are internal layers behind the A2UI adapter. The
model preserves surface IDs, component IDs, resolved accessibility attributes, state bindings, and
action context while keeping platform component selection inside the host.
...
1. **Implemented for the documented boundary:** preserve official tools, content blocks, resources, schemas, annotations, `_meta`, and cache hints without a lossy parallel content shape.
...
9. **Implemented beginning in the `0.7.0` release:** an official temporary React Native host type-checks and bundles the accessibility and complete non-media fixtures through primitive, typed-adapter, and closed-variant catalogs. CI pins this generated host for reproducible integration evidence rather than using it as a package version boundary. The fixture intentionally uses a no-network image placeholder because the raw React Native image primitive cannot enforce the declared redirect/decode/cache grant. The Expo Go todo app provides additional application-level validation.
10. **Implemented in the `0.8.0` release:** policy-gated `Video` and `AudioPlayer`, exact host-extension negotiation/manifests/registrations, hostile and amplification tests, and one generated Codegen/Fabric component backed by UIKit and Android View. Extension nodes are leaves: structural child fields and generic A2UI actions are rejected rather than ignored. Parsed schemas, validated semantic props, and policy requests are immutable snapshots. The fixture intentionally does not fetch media because the installed player must enforce the complete grant.
11. **Current native-host dependency policy:** React `>=18.1.0` is the only renderer peer. Native components, platform integrations, and application frameworks are supplied by the host; Expo and React Native are not package dependencies. Generated-host CI bundles and natively builds a pinned integration fixture on pull requests.
12. **Implemented in the `0.9.0` release:** the generated reference host combines native A2UI, a local Fabric extension, and an isolated MCP Apps sibling. Its host-owned coordinator distributes lifecycle and environment state without merging protocol, WebView-security, or accessibility-tree boundaries; see the [support policy](support-matrix.md).
...
7. **Verified package boundary:** all 25 scored authorization client scenarios in the exact pinned
   official `2026-07-28` requirements fixture pass with no expected failures. The Expo Go todo app
   provides separate native application evidence; broader host controls remain host
   responsibilities.
