...
| MCP authorization   | `2026-07-28` OAuth discovery, PKCE, issuer/resource binding, least-privilege scopes, and bearer usage     | Issuer-bound secure-store/callback provider and credential-safe transport factory passing all 25 scored pinned authorization client scenarios; native integration is demonstrated separately in app-level PoCs                                                                                               | Verified package boundary               |
...
| Core tool consent   | Product policy rather than a server-granted protocol capability                                           | Core `McpNativeRuntime.dispatch()` supports bounded per-action review over explicit host-authored tool/argument matches, risk, capabilities, sensitive-data use, and external sharing; other tool/action boundaries still require host integration                                                           | Implemented core foundation             |
...
9. **Implemented for the `0.4.0` scope:** official temporary React Native `0.87.1` and `0.86.3` hosts type-check and bundle the fixture through primitive, typed-adapter, and closed-variant catalogs. Platform and library-specific validation proceeds independently in the non-blocking Expo Go PoCs.
