...
9. **Partial:** official temporary React Native `0.87.1` and `0.86.3` hosts type-check and bundle the fixture through primitive, typed-adapter, and closed-variant catalogs; Android/iOS native build preflight, a WCAG 2.2 applicability assessment, and a strict evidence validator are defined. The Android 17 Google Play emulator row passes with TalkBack evidence. Six required physical VoiceOver/TalkBack rows remain unexecuted and block the `0.4.0` release gate.
