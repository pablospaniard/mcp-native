...
9. **Implemented beginning in the `0.7.0` release:** an official temporary React Native host type-checks and bundles the accessibility and complete non-media fixtures through primitive, typed-adapter, and closed-variant catalogs. The current automated baseline is the declared `0.86.0` minimum. The fixture intentionally uses a no-network image placeholder because the raw React Native image primitive cannot enforce the declared redirect/decode/cache grant. The Expo Go todo app provides additional application-level validation.
...
11. **Current React Native support policy:** React Native `>=0.86.0 <1` is supported. Generated-host CI bundles and natively builds the exact declared `0.86.0` minimum on pull requests so the compatibility floor cannot move accidentally. Lack of a dedicated lane for another version or JavaScript engine is not an incompatibility claim.
