...
verified by the current release candidate. Use it to see the implemented coverage, normative pins,
...
provides a runnable native application proof. See the [MCP protocol support policy](protocol-support.md), [A2UI
...
The package implements the documented v1 Candidate profile. Its custom `0.1` input is deprecated,
frozen, and isolated for migration; it is a separate legacy model rather than an A2UI protocol
version. The v1 path is reported against the exact [feature-scoped conformance profile](a2ui-v1-conformance.md).

The legacy `application/a2ui+json` resource convention came from earlier A2UI-over-MCP work. A2UI
v1.0 is transport-agnostic and defines a stream of protocol envelopes; the current path combines
that envelope model with the negotiated project binding below.
