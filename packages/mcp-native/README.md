...
The supported A2UI v1 native subset now includes renderer-local absolute and dynamic-list-relative string bindings, bounded `formatString`, host-localized number, currency, date, and plural formatting, validation functions and supported renderer checks, pure boolean functions, template-instance action resolution, `@index` with offsets, host-callback action envelopes, policy-gated HTTP(S) `openUrl`, and closed catalog-capability metadata with inline catalogs disabled. The core runtime now also exports a bounded consent policy for its `McpNativeRuntime.dispatch()` path with explicit host-authored risk, capability, sensitive-data, and external-sharing descriptors; direct tool calls, MCP Apps callbacks, and A2UI v1 action delivery still require separate host integration. The separately installed `@mcp-native/mcp` adapter includes an issuer-bound interactive OAuth boundary for protected HTTP that passes every scored pinned `2026-07-28` authorization client scenario, plus dependency-neutral platform secret-store/auth-session reference adapters. Persistent consent and production lifecycle controls, full streaming host integration, transport placement for A2UI capability objects, and the separate Expo Go integration PoCs remain incomplete. Follow the [roadmap](https://github.com/pablospaniard/mcp-native/blob/main/docs/roadmap.md) for progress.
