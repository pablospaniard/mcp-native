...
- Clarify repository-wide A2UI claims: MCP Native supports a pinned, feature-scoped profile of
  ...
  coverage, and make unsupported RPC messages and reviewed revision updates explicit.
...
- Contain thrown and rejected React Native error observers, including direct registered-surface
  observers, so a server-triggered render, session, retry, startup, or teardown failure cannot
  escape its fail-closed host boundary.
